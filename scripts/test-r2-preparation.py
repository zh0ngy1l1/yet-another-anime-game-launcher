...
with tempfile.TemporaryDirectory(prefix='yaagl-r2-tests-') as tmp:
    ...
    (source / 'unexpected-dangling').symlink_to('missing')
    invoke('unexpected-dangling-link-rejected', False)
    (source / 'unexpected-dangling').unlink()
    for name, target in manifest['archiveDanglingLinks'].items():
        link = source / name.lstrip('/')
        link.parent.mkdir(parents=True, exist_ok=True)
        link.symlink_to(target)
    invoke('exact-public-archive-dangling-links-preserved', True)
    link.unlink(); link.symlink_to('unexpected-target')
    invoke('changed-archive-dangling-target-rejected', False)
