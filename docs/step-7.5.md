...
3. **Disabled launch.** Disable unlocking and leave the target saved. In Settings → Game keep **Enable Steam Patch on**, **Launch Fix(block hosts) off**, and HDR/custom resolution off for this baseline. Record Timeout Fix/Metal HUD and graphics choices. Launch once. The game directory still points to the **shared** `/Users/david/.gimpact`; copied Wine/prefix does not copy game files. Keep other launchers away from those files. Use the [read-only before/after file snapshot and log checklist](step7-verification.md#next-operator-checkpoint-disabled-baseline-only). Confirm ordinary upstream behavior, upstream game DXMT maximum 60 (or its ordinary non-DXMT environment), zero FPS bridge acquisition/hash checks/helper requests, and no `fps-bridge.exe` associated with this request. Log inspection must distinguish ordinary DXMT/resource acquisition from FPS-specific acquisition. Exit the game normally. Confirm request-owned Wine waiting and config/patch cleanup complete before the primary action becomes available. Check settings remain usable.

4. **Separate enabled 60, 61 and 120 launches.** Use the existing supported DXMT Wine metadata; keep **Enable Steam Patch on** and **Launch Fix(block hosts) off**. The source does not change either persisted setting. Start from the normal terminal, outside inherited `SteamGameId`/Proton service mode. Save one target and launch it, then complete step 5 before the next target. Send the disabled result first, then the enabled-60 result with the full launcher log, game/Steam/DXMT logs, terminal output, settings/graphics and measured-FPS observations, before/after file evidence, all lifecycle transitions and any retained journal/registry/mailbox paths. Wait for review before 61/120. Inspect `yaaglwdos/neutralinojs.log` (China: `yaaglwd/neutralinojs.log`) for the request token, exact staged artifact, explicit loader/prefix, target and both environment values:
   ...
   The helper is now a thread in the bridge. Its argument is the generation-bound protocol `start` argument, not an unrelated `unlockfps.exe` process command line. Confirm `route=steam-patch`, `steamReady:1`, distinct stable `shimPid`/game `pid`, `launched:1`, worker generation 1 and `workerState:2` after discovery/initialization. The exact staged executable is `/tmp/yaagl-fps.<request nonce>/fps-bridge.exe`, identified in the log; its digest must match the manifest. The same request directory contains the unchanged signed `steam.exe` (111304 bytes, SHA-256 `0424339444c54bf1f9fdbadf12e4e2c90ceef41d987fe573b93f5f2ebfd8a657`) and `lsteamclient.dll` (5560872 bytes, SHA-256 `af50ed0d952ef98d99d4d3ff67b4836b545c9403894430fca31969f9f630637b`). Read its `response` file only; do not write protocol files in a real-game test.
...
5. **Normal exit, cleanup and relaunch.** Exit via the game's normal UI after each case. Verify original game `primaryExited:1`, game job `active:0`, Steam `shimExited:1` and `steamActive:0`, `steamError:0`, worker `workerDone:1`, bridge release, confirmed supervisor/direct-child completion, the request-owned Wine-wait completion messages, confirmed registry restoration, the following registry-command Wine-wait completion, and `registry/file restoration and private resource cleanup completed`. A vanished window or a command-issuance log alone is not proof. `config.bat` and other temporary files should be removed if newly created or restored if pre-existing. Check original HDR/resolution/Wine registry values and original patch files, including pre-existing `.bak` files, against the recorded snapshots. The private request directory is removed only after safe restoration.

   Main logs are `yaaglwdos/neutralinojs.log`; game output is `yaaglwdos/logs/game_<timestamp>.log`; enabled Steam output is the adjacent `game_<timestamp>.log.steam.log`; DXMT logs/config are rooted in `yaaglwdos` by `DXMT_LOG_PATH`/`DXMT_CONFIG_FILE`. China substitutes `yaaglwd`. Failed/unconfirmed requests log retained `/tmp/yaagl-fps.<nonce>` and `/tmp/yaagl-owned-wine.<nonce>` paths; preserve their response/journal/registry evidence. Do not remove those directories during unresolved cleanup.

   Safe read-only inspection, in another terminal:

   ```sh
   /bin/ps -axo pid,ppid,lstart,command | rg 'fps-bridge\.exe|GenshinImpact\.exe|YuanShen\.exe|steam\.exe|supervisor\.pl|wineserver'
...
For this fix’s harmless Steam boundary and production-RPC regressions (never a real game):

```sh
npm exec --yes --package=node@16.20.2 --package=pnpm@7.33.7 --call 'FPS_TEST_WINE="/Users/david/Library/Application Support/Yaagl OS/wine/bin/wine" node scripts/test-fps-bridge.cjs --steam'
npm exec --yes --package=node@16.20.2 --package=pnpm@7.33.7 --call 'pnpm exec vite build --mode=development --config scripts/runtime-fixture.config.ts'
npm exec --yes --package=node@16.20.2 --package=pnpm@7.33.7 --call 'FPS_TEST_WINE="/Users/david/Library/Application Support/Yaagl OS/wine/bin/wine" node scripts/test-runtime-fixture.cjs --rpc --steam --no-handoff'
npm exec --yes --package=node@16.20.2 --package=pnpm@7.33.7 --call 'FPS_TEST_WINE="/Users/david/Library/Application Support/Yaagl OS/wine/bin/wine" node scripts/test-runtime-fixture.cjs --rpc --steam'
npm exec --yes --package=node@16.20.2 --package=pnpm@7.33.7 --call 'FPS_TEST_WINE="/Users/david/Library/Application Support/Yaagl OS/wine/bin/wine" node scripts/test-runtime-fixture.cjs --rpc --steam --tamper-steam --no-handoff'
```

Each run creates disposable fixture prefixes; none uses the copied development prefix as scratch space. Fixture passes establish neither Genshin world load nor measured game FPS. Packaged-app testing remains after Step 8.
