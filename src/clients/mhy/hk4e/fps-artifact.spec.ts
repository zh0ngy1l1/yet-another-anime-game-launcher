...
import { FPS_BRIDGE_MANIFEST as manifest } from "./fps-bridge-manifest";
...
const directory = "/tmp/yaagl-request";
const source = "/application/sidecar/fps-bridge/fps-bridge.exe";
...
function fixture(cached?: File) {
  ...
  const io = {
    ...
    copy: vi.fn(async (_url: string, path: string) => {
...
describe("HK4E FPS artifact acquisition", () => {
  it("verifies a cache hit without copying, rewriting, deleting, or promoting", async () => {
    const { io, events } = fixture(goodFile());
    await expect(
      stageFpsArtifact(directory, source, manifest, io)
    ...
    for (const effect of [io.temporary, io.copy, io.promote, io.cleanup]) {
  ...
  it.each(["absent", "wrong size", "wrong hash"])(
    ...
    async state => {
      ...
      expect(io.copy).toHaveBeenCalledWith(source, temporary);
  ...
  it.each([
    ...
    io.copy.mockImplementation(async (_url, path) => {
      const file = goodFile();
    ...
    await expect(
      stageFpsArtifact(directory, source, manifest, io)
    ).rejects.toThrow("verify temporary file");
  ...
  it.each(["copy interrupted", "copy rejected", "promotion failed"])(
    ...
    async message => {
      ...
      else io.copy.mockRejectedValue(cause);
      await expect(
        stageFpsArtifact(directory, source, manifest, io)
  ...
  it.each([
    ...
    await expect(
      stageFpsArtifact(directory, source, manifest, io)
    ).rejects.toThrow("verify final file");
  ...
  it("rejects temporary creation failure without cleaning an unowned path", async () => {
    ...
    await expect(
      stageFpsArtifact(directory, source, manifest, io)
    ).rejects.toThrow("create temporary file");
    expect(io.cleanup).not.toHaveBeenCalled();
    expect(io.copy).not.toHaveBeenCalled();
  });
  ...
  it.each(["", `${manifest.sha256}\nextra`])(
    ...
    async output => {
      ...
      io.sha256.mockImplementation(path =>
        fpsArtifactIO.sha256(path, manifest.size)
      );
      await expect(
        stageFpsArtifact(directory, source, manifest, io)
  ...
  it.each([false, true])(
    ...
    async loggingFails => {
      const { io, temporary } = fixture();
      const cause = new Error("interrupted copy");
      const cleanupError = new Error("cleanup denied");
      const loggingError = new Error("log denied");
      io.copy.mockRejectedValue(cause);
      io.cleanup.mockRejectedValue(cleanupError);
  ...
  it.each(["success", "failure"])(
    ...
    async outcome => {
      ...
      io.copy.mockImplementation(async (_source, path) => {
        if (path === temporary) {
          entered.resolve();
          await release.promise;
          if (outcome === "failure") throw new Error("first copy interrupted");
        }
        events.push(`copy:${path}`);
        files.set(path, goodFile());
      });
      const first = stageFpsArtifact(directory, source, manifest, io).then(
        value => ({ value }),
        error => ({ error })
      );
      ...
      expect(io.copy.mock.calls.map(call => call[1])).toEqual([
        temporary,
        secondTemporary,
      ]);
  ...
  it("a caller whose promotion fails rejects even if a winner installed valid bytes", async () => {
    ...
    await expect(
      stageFpsArtifact(directory, source, manifest, io)
    ).rejects.toThrow("atomically promote");
    expect(io.cleanup.mock.calls).toEqual([[temporary]]);
    expect(files.get(finalPath)).toEqual(goodFile());
  });
});
...
describe("FPS artifact native boundaries", () => {
  it.each([
    manifest.sha256,
    ...
    nativeMock(output);
    await expect(fpsArtifactIO.sha256(finalPath, manifest.size)).resolves.toBe(
      manifest.sha256
    );
  });
  ...
  it.each([
    ...
    await expect(fpsArtifactIO.sha256(path, manifest.size)).rejects.toThrow(
  ...
  it("reserves a file in the exact cache directory", async () => {
    ...
    await expect(fpsArtifactIO.temporary(directory)).resolves.toBe(path);
  ...
  it.each([
    ...
    async output => {
      ...
      await expect(fpsArtifactIO.temporary(directory)).rejects.toThrow(
        "Invalid FPS artifact temporary path"
      );
      expect(execCommand).toHaveBeenCalledTimes(2);
    }
  );

  it("copies only the selected local artifact", async () => {
    const { execCommand, spawnProcess } = nativeMock();
    await fpsArtifactIO.copy(source, finalPath);
    expect(execCommand).toHaveBeenCalledWith(
      build(["/bin/cp", "--", source, finalPath]),
      {}
    );
  ...
  it("rejects cross-directory promotion", async () => {
    const { execCommand } = nativeMock();
    await expect(
      fpsArtifactIO.promote("/tmp/source", finalPath)
  ...
  it("a default cache hit executes only the trusted digest utility, never the artifact or Wine", async () => {
    ...
    await expect(stageFpsArtifact(directory, source, manifest)).resolves.toBe(
      finalPath
    );
  ...
  it("default acquisition uses only filesystem, digest, and copy commands and ends with final hashing", async () => {
    const { execCommand, spawnProcess } = nativeMock();
    const path = join(directory, ".unlockfps-aB01234567");
  ...
  it("streams a tiny file and directly replaces a file, refusing symlinks and directories", async () => {
    const root = await mkdtemp(join(tmpdir(), "yaagl-artifact-test-"));
    ...
    try {
      await mkdir(hostile);
      const first = await fpsArtifactIO.temporary(hostile);
      const second = await fpsArtifactIO.temporary(hostile);
      expect(first).not.toBe(second);
      expect(dirname(first)).toBe(hostile);
      await writeFile(first, "abc");
      await expect(fpsArtifactIO.sha256(first, 3)).resolves.toBe(
        "ba7816bf8f01cfea414140de5dae2223b00361a396177a9cb410ff61f20015ad"
      );
      await expect(fpsArtifactIO.sha256(first, 4)).rejects.toThrow();
      const link = join(hostile, "link");
      await symlink(first, link);
      await expect(fpsArtifactIO.sha256(link, 3)).rejects.toThrow();
      await expect(fpsArtifactIO.sha256(hostile, 3)).rejects.toThrow();
      const inode = (await lstat(first)).ino;
      await writeFile(second, "old");
      await fpsArtifactIO.promote(first, second);
      ...
      await expect(
        fpsArtifactIO.promote(second, destinationDirectory)
      ).rejects.toThrow();
      expect(await readFile(second, "utf8")).toBe("abc");
      await fpsArtifactIO.cleanup(second);
      await fpsArtifactIO.cleanup(second); // missing owned file is harmless
