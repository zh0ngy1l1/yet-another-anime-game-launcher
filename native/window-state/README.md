...
Commands: `save|apply|observe|restore|discard DIRECTORY SERVER FULLSCREEN WIDTH HEIGHT CLAMP`.
...
mask. A durable saved-phase marker precedes apply. Restore requires the original
snapshot; a missing snapshot fails closed. Discard permits a missing snapshot
only before any apply was admitted. Restore is idempotent, preserves raw types/bytes,
removes absent values and only deletes newly created keys/ancestors if still empty. Unrelated values and
